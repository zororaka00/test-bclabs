...
export class EmailClass {
  ...
  async sendAlert(token_string: string, increasePercentage: number): Promise<void> {
  ...
  async sendPriceAlert(email: string, token_string: string, currentPrice: number): Promise<void> {
    const senderEmail = this.configService.get<string>('GMAIL_EMAIL');
    const senderName = this.configService.get<string>('GMAIL_NAME');
