        ...
        if (!this.stateLoopPrice) {
            ...
            try {
                ...
                if (prices && prices.length > 0) {
                    const passingPriceLogs = await Promise.all(prices.map(async data => ({
                        token_id: (await getTokenPrices.find(token => token.token_address.toLowerCase() == data.tokenAddress.toLowerCase())).id,
                        price: data.usdPrice
                    })));
                    ...
                    const logs1HourAgo = await this.prismaService.tokenPriceLogs.findMany({
                        where: {
                            created_at: {
                                ...
                                lte: new Date(new Date().setMinutes(new Date().getMinutes() - 55))
                            }
                        },
                        orderBy: {
                            created_at: 'asc'
                    ...
                    if (logs1HourAgo && logs1HourAgo.length > 0) {
                        await Promise.all(passingPriceLogs.map(async data => {
                            const fIndex = await logs1HourAgo.findIndex(log => log.token_id == data.token_id);
                            if (fIndex > -1) {
                                const oldPrice = Number(logs1HourAgo[fIndex].price);
                                const priceChange = ((data.price - oldPrice) / oldPrice) * 100;
                                ...
                                if (isMore3Percent) {
                                    ...
                                    await this.helperService.sendAlert(token_string, priceChange);
                                }
                            }
                        }));
                        this.stateLoopPrice = true;
        ...
        if (!this.stateLoopTargetPrice) {
            ...
            try {
                ...
                if (getAlerts && getAlerts.length > 0) {
                    await Promise.all(getAlerts.map(async data => {
                        const isMoreTargetPrice = data.token.tokenPriceLogs[0].price >= data.target_price;
                        if (isMoreTargetPrice) {
                            await Promise.all([
                                this.helperService.sendPriceAlert(data.email, `${data.token.token_name} (${data.token.token_symbol})`, Number(data.target_price)),
